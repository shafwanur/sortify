...
def sort_songs(arg: str):
    ...
    if arg == "album-sort": 
        globalvars.song_list = sorted(globalvars.song_list, key = lambda x: (x["release_date"], x["album_name"], -x["popularity"]))
    if arg == "global-sort": 
        globalvars.song_list = sorted(globalvars.song_list, key = lambda x : -x["popularity"])
    if arg == "all-of": 
        globalvars.song_list = sorted(globalvars.song_list, key = lambda x : (x["release_date"], x["album_name"]))
